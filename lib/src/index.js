...
class PaperCollateralRenderer extends Component {
  ...
  extendWallet(wallets) {
    ...
    console.log(JSON.stringify(flat(walletsClone[0]), null, '  '))

    return walletsClone
  }

  preprocess(templates, extendedWallet) {
  ...
  componentDidMount() {
    ...
    const extendedWallets = this.extendWallet(this.props.wallet)
    const filledTemplates = extendedWallets.map(wallet => {
      return this.preprocess(templates, wallet)
    })
