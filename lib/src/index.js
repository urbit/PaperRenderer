...
// We need a size to load image assets at. This is a hacky approximation, because it does not reflect the actual size of image targets provided by figma.
...
class PaperCollateralRenderer extends Component {
  ...
  preprocess(wallets, layouts) {
    // map thru wallets
    wallets.map(wallet => {
      // for each wallet, map through layouts and fill data
      layouts.map(layout => {
  ...
  componentDidMount() {
    // initialize canvas of the size needed. The size is dependant on the display resolution that the user is using.
    this.canvas = initCanvas(this.canvasRef.current, { x: 612, y: 792 }, 4);

    // Takes the wallets, and generates any async assets like sigils and qr codes.
    const extendedWallets = this.extendWallets(this.props.wallets)

    // applies data from extendWallets to the layouts.
    const pages = this.preprocess(extendedWallets, layouts)
