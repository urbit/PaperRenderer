...
const MasterTicketComponent = async (wallet, constants, templates) => {
  ...
  const TEMPLATE = `MASTER_TICKET:${toUpperCase(retrieve(wallet, 'meta.class'))}`

  const props = {
    heading: 'Master Ticket',
    patp: retrieve(wallet, 'meta.patp'),
    sigil: await loadSigil(AT_LOAD_SIGIL_SIZE, retrieve(wallet, 'meta.patp')),
    ticket: {
      data: retrieve(wallet, 'ticket'),
      size: getTicketSize(KEY, retrieve(wallet, 'meta.class')),
    },
    ownership: {
      seed: {
        mnemonic: retrieve(wallet, 'ownermeta.seed'),
        size: SEEDSIZE,
        derivationPath: BIP32_DERIVATION_PATH,
      },
      ethereum: {
        address: retrieve(wallet, 'ownermeta.keys.address'),
        qr: await loadQR(AT_LOAD_QR_SIZE, retrieve(wallet, 'ownermeta.keys.address')),
      },
    },
    meta: {
    ...
    _classOf: retrieve(wallet, 'meta.class'),
    _type: KEY,
    _bin: assignBin(retrieve(wallet, 'meta.class'), KEY)
  };
...
const MasterTicketShardsComponent = async (wallet, constants, templates) => {
  const pages = await wallet.shards.map(async (shard, index) => {
    ...
    const TEMPLATE = `MASTER_TICKET_SHARD:${toUpperCase(retrieve(wallet, 'meta.class'))}`;
    const props = {
      ...
      shard: {
        ...
        data: shard,
        size: getTicketSize('masterTicketShard', retrieve(wallet, 'meta.class')),
      },
      ownership: {
        ethereum: {
      ...
      copy: {
        // custody: retrieve(constants, `custody.${[wallet.meta.class]}.${[KEY]}`),
        // usage: retrieve(constants, `usage.${[wallet.meta.class]}.${[KEY]}`),
      },
      meta: {
        createdOn: dateToDa(new Date()),
        walletVersion: constants.meta.walletVersion,
...
const SpawnSeedComponent = async (wallet, constants, templates) => {
  const KEY = 'spawn';
  const TEMPLATE = `SPAWN:${toUpperCase(retrieve(wallet, 'meta.class'))}`
...
const VotingSeedComponent = async (wallet, constants, templates) => {
  const KEY = 'voting';
  const TEMPLATE = `VOTING:${toUpperCase(retrieve(wallet, 'meta.class'))}`
...
const ManagementSeedComponent = async (wallet, constants, templates) => {
  ...
  const TEMPLATE = `MANAGEMENT:${toUpperCase(retrieve(wallet, 'meta.class'))}`
