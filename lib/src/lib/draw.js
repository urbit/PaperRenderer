...
const qr = ({ ctx, img, x, y, size, type }) =>
  ctx.drawImage(img, x, y + 3, size, size)

const sigil = ({ ctx, img, x, y, size, type }) =>
  ctx.drawImage(img, x, y, size, size)

const img = ({ ctx, img, x, y, width, height, type }) =>
...
const ethereumAddressLong = (
  ctx,
  { fontWeight, fontSize, lineHeightPx, x, y, fontFamily, text, fontColor }
) => {
...
const patq = (ctx, { fontSize, lineHeightPx, x, y, fontFamily, text }) => {
