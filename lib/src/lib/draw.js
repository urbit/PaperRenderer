...
const scalePath = (path, x, y) => {
  var p = path.split(' ').replace(/\s/g, '')
...
const img = (ctx, { type, draw, data, path, width, height, x, y, color }) => {
  ...
  // reset translation with same offset as index.js initCanvas
  ctx.setTransform(1, 0, 0, 1, 0, 0)
  ctx.translate(0.5, 0.5)
  ctx.scale(4, 4)
}

const loadFont = async (fontFamily, fontWeight) => {
  const font = new FontFace('fontFamily', ``, {
    style: 'Regular',
    weight: fontWeight,
  })
  ...
  font.loaded.then(
    (fontFace) => {
      console.log(`Loaded: ${fontFamily} ${fontWeight}`)
    },
    (fontFace) => {
      // console.log(`Unable to load: ${fontFamily} ${fontWeight}`)
    }
  )
}
...
const wrappedText = (
  ...
  loadFont(fontFamily, fontWeight).then(
    () => {
      // console.log("Loaded succeeded")
    },
    () => {
  ...
  wordWrap(ctx, data, x, y + lineHeightPx, lineHeightPx, maxWidth, fontColor)
}
...
const patq = (ctx, { fontSize, lineHeightPx, x, y, fontFamily, data }) => {
  const OFFSET = 1.2
